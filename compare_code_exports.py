...
def read_table(path: str) -> pd.DataFrame:
    ...
    if ext in [".xlsx", ".xls"]:
        # Explicit engine avoids environment weirdness
        return pd.read_excel(path, dtype=str, keep_default_na=False, engine="openpyxl")
    raise ValueError(f"Unsupported file type: {ext}")


def _strip_inline_abap_comment_safish(line: str) -> str:
    """
    Naive but safer-ish ABAP inline comment stripping:
    Removes everything after a double-quote comment marker (")
    but tries not to strip if inside a single-quoted string.

    It is still not a full ABAP parser, but avoids some common false positives.
    """
    s = line
    in_single = False
    i = 0
    while i < len(s):
        ch = s[i]
        if ch == "'":
            # ABAP escapes single quotes inside strings by doubling ''
...
def normalize_code(code: str, mode: str, ignore_blank_lines: bool = False) -> str:
    if code is None:
        return ""
    s = str(code).replace("\r\n", "\n").replace("\r", "\n")
    ...
    elif mode == "strip_comments_abap":
        # Remove full-line comments starting with *
        # and remove inline " comments (naive but safer-ish)
        lines = []
        for line in s.split("\n"):
            raw = line.rstrip()
            if raw.lstrip().startswith("*"):
                continue
            raw = _strip_inline_abap_comment_safish(raw)
            lines.append(raw)
        out = "\n".join(lines)
...
def build_key(df: pd.DataFrame, key_cols: list[str]) -> pd.Series:
    ...
    # Safer key encoding: JSON array of parts avoids "||" collisions.
    # Example: ["ZCL_FOO", "MAIN"] -> '["ZCL_FOO","MAIN"]'
    parts_df = df[key_cols].fillna("").astype(str)

    def encode_row(row) -> str:
        parts = [str(v).strip() for v in row.tolist()]
        return json.dumps(parts, ensure_ascii=False, separators=(",", ":"))

    return parts_df.apply(encode_row, axis=1)
...
def filter_df(df: pd.DataFrame, author_col: str, object_col: str, persons: list[str], objects: list[str]) -> pd.DataFrame:
    ...
    if objects:
        # NOTE: Kept as-is (per your request: not changing #5 behavior)
        # This is exact match against object_col only.
...
def main():
    ...
    ap.add_argument("--ignore-blank-lines", action="store_true",
                    help="Ignore blank lines after normalization")
    ...
    merged["code_old_norm"] = merged[old_code_col].apply(lambda s: normalize_code(s, args.normalize, args.ignore_blank_lines))
    merged["code_new_norm"] = merged[new_code_col].apply(lambda s: normalize_code(s, args.normalize, args.ignore_blank_lines))
